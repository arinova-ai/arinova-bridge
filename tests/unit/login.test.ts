...
describe("login", () => {
  ...
  it("exits when no providers are enabled", async () => {
    mockReadConfigFile.mockReturnValue(
      createConfig([{ id: "p1", type: "anthropic-cli", displayName: "P1", enabled: false }]),
    );
  ...
  it("exits when providerId not found in enabled providers", async () => {
    mockReadConfigFile.mockReturnValue(
      createConfig([{ id: "p1", type: "anthropic-cli", displayName: "P1", enabled: true }]),
    );
  ...
  it("auto-selects the single loginable provider without prompting", async () => {
    ...
    mockReadConfigFile.mockReturnValue(
      createConfig([{ id: "solo", type: "anthropic-cli", displayName: "Solo Provider", enabled: true, configDir }]),
    );
    ...
    expect(mockSelect).not.toHaveBeenCalled();
    expect(mockSpawnSync).toHaveBeenCalledWith("claude", ["login"], expect.objectContaining({ stdio: "inherit" }));
  });
  ...
  it("passes --device-auth flag for openai-cli when deviceAuth option is true", async () => {
    ...
    mockReadConfigFile.mockReturnValue(
      createConfig([{ id: "openai-dev", type: "openai-cli", displayName: "OpenAI Device", enabled: true, configDir }]),
    );
      ...
      createConfig([
        {
          ...
          enabled: true,
          configDir,
          codexPath: "/usr/local/bin/my-codex",
        },
      ]),
    );
    ...
    mockReadConfigFile.mockReturnValue(
      createConfig([
        { id: "anth-prov", type: "anthropic-cli", displayName: "Anthropic", enabled: true, configDir },
        {
          ...
          displayName: "OpenAI",
          enabled: true,
          configDir: path.join(tmpDir, "openai-status"),
        },
      ]),
    );
  ...
  it("shows minimax provider status with valid token", async () => {
    ...
    mockSelect.mockResolvedValueOnce("mm1").mockResolvedValueOnce("global");
    ...
    mockReadConfigFile.mockReturnValue(
      createConfig([
        { id: "openai1", type: "openai-cli", displayName: "OpenAI 1", enabled: true, configDir },
        {
          ...
          enabled: true,
          configDir: path.join(tmpDir, "openai2"),
        },
      ]),
    );
    ...
    mockReadConfigFile.mockReturnValue(
      createConfig([
        { id: "openai-bad", type: "openai-cli", displayName: "OpenAI Bad JWT", enabled: true, configDir },
        {
          id: "openai-other",
          type: "openai-cli",
          displayName: "OpenAI Other",
          enabled: true,
          configDir: path.join(tmpDir, "other"),
        },
      ]),
    );
  ...
  it("does not create configDir when provider has no configDir set", async () => {
    mockReadConfigFile.mockReturnValue(
      createConfig([{ id: "no-dir", type: "anthropic-cli", displayName: "No Dir", enabled: true }]),
    );
